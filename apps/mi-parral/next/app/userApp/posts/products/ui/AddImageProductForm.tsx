...
import { useUploadProductImage } from '@/app/hooks/useUploadProductImageHook';
import ImageSkeleton from '@/components/ImageSkeleton';

interface AddImageProductFormProps {
  onClose: () => void;
  serviceId: string;
}

export default function AddImageProductForm({
  onClose,
  serviceId,
}: AddImageProductFormProps) {
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [file, setFile] = useState<File | null>(null);
  const { isLoading, error, uploadImage } = useUploadProductImage();
