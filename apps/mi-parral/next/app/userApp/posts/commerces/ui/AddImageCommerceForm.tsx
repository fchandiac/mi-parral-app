...
import { useUploadCommerceImage } from '@/app/hooks/useUploadCommerceImage';
...
export default function AddImageCommerceForm({
  onClose,
  commerceId,
}: AddImageServiceFormProps) {
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [file, setFile] = useState<File | null>(null);
  const { isLoading, error, uploadImage } = useUploadCommerceImage();

  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (file && commerceId) {
      const upload = await uploadImage(file, commerceId);
      onClose();
    }
  };
